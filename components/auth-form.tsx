...
export function AuthForm({ mode, returnTo = '/my-learning' }: { mode: 'login' | 'signup'; returnTo?: string }) {
  ...
  async function submit(event: { preventDefault(): void; currentTarget: HTMLFormElement }) {
    ...
    if (response.ok) location.href = body.redirect || returnTo;
    else { setError(body.error || 'Unable to continue'); setBusy(false); }
  }
  return <form onSubmit={submit} className="mt-7 space-y-4">
    ...
    <button disabled={busy} className="w-full rounded-lg bg-[#0757B2] px-4 py-3 font-semibold text-white disabled:opacity-60">{busy ? 'Please wait…' : mode === 'login' ? 'Log in' : 'Create account'}</button>
