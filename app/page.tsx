        ...
        <section className="border-b bg-white">
          <div className="mx-auto max-w-[1440px] px-5 py-16 lg:px-10">
            <div className="flex flex-wrap items-end justify-between gap-4">
              <div><p className="section-kicker">Popular courses</p><h2 className="section-title">Skills learners are building now</h2></div>
            ...
            <div className="mt-8 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
              {currentCourses.slice(3,9).map((course)=><a key={course.id} href={`/course/${course.slug}`} className="group overflow-hidden rounded-xl border bg-white shadow-sm hover:shadow-lg"><div className="relative aspect-video overflow-hidden"><Image src={courseCover(course.slug)} alt="" fill sizes="(max-width: 1024px) 50vw, 33vw" className="object-cover transition duration-500 group-hover:scale-105"/><div className="absolute inset-0 bg-gradient-to-t from-[#061b3a]/80 to-transparent"/><span className="absolute left-4 top-4 rounded-full bg-white/90 px-2.5 py-1 text-xs font-semibold text-[#073D86]">Popular · {course.category}</span><h3 className="absolute bottom-4 left-5 font-heading text-lg font-bold text-white">{course.shortTitle}</h3></div><div className="p-5"><p className="line-clamp-2 min-h-12 text-sm leading-6 text-[#5d696c]">{course.description}</p><div className="mt-4 flex items-center justify-between border-t pt-4 text-sm"><span>{(enrollmentCounts[course.id]?.total ?? 0).toLocaleString()} enrolled</span><b>{formatPrice(course)}</b></div></div></a>)}
            </div>
          </div>
        </section>
        <section
